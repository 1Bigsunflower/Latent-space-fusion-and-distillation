...
# === 配置 ===
folder = os.path.join(script_dir, "megnet_human_ari")
output_file = os.path.join(script_dir, "megnet_hm_ari.csv")
...
with open(output_path, 'w', newline='') as out_csv:
    ...
    for dim in dims:
        for task in tasks:
            for fold in folds:
                filename = f"{task}_hm_fold{fold}_dim{dim}.csv"
                filepath = os.path.join(folder, filename)

                with open(filepath, 'r') as f:
                    reader = csv.DictReader(f)
                    row = next(reader)
                    row_with_filename = {"task": f"{task}_hm_fold{fold}_dim{dim}"}
...
print(f"合并完成，总表保存在: {output_path}")
