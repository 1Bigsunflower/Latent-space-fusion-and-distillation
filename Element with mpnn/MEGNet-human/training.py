    ...
    def extract_atom_embeddings(self, dataloader, save_path, split_name):
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(dataloader):
                ...
                node_feat = g.ndata["node_type"].to(device)
                edge_feat = g.edata["edge_attr"].to(device)
                node_feats = self.model(
                    g,
                    edge_feat,
                    node_feat,
                    state_attr,
                ...
                graphs = dgl.unbatch(g)  # 拆成 list of subgraphs
                node_to_graph = []
                for i, subgraph in enumerate(graphs):
                    node_to_graph.extend([i] * subgraph.num_nodes())
                node_to_graph = torch.tensor(node_to_graph, device=device)

                expanded_target = labels[node_to_graph]

                batch_data = {
                    "node_input": node_feats[0].cpu(),
                    "node_embedded": node_feats[1].cpu(),
                    "node_linear": node_feats[2].cpu(),
                    "node_encoded": node_feats[3].cpu(),
                    "block_1": node_feats[4].cpu(),
                    "block_2": node_feats[5].cpu(),
                    "block_3": node_feats[6].cpu(),
                    "target": expanded_target.cpu(),
                    "node_to_graph": node_to_graph.cpu()
                }

                all_batches.append(batch_data)

        save_path_full = os.path.join(save_path, f"{split_name}.pt")
        torch.save(all_batches, save_path_full)
