...
# 邻接矩阵与谱分析
# ---------------------------
def build_adjacency_matrix(embeddings, alpha=None):
    n = len(embeddings)
    distances = compute_distance_matrix(embeddings)
    if alpha is None:
        mean_dist = np.mean(distances[distances > 0])
        alpha = -np.log(0.5) / (mean_dist ** 2)
    A = np.zeros((n, n))
    for i in range(n):
        for j in range(i + 1, n):
            aij = np.exp(-alpha * (distances[i, j] ** 2))
            A[i, j] = aij
            A[j, i] = aij
...
def clustering_metrics(embeddings, element_embeddings):
    ...
    silhouette = silhouette_score(np.vstack(list(element_embeddings.values())), cluster_labels)
...
def form(model, h_or_e):
    ...
    for dim_idx, dim in enumerate(dims):
        for task_idx, task_name in enumerate(task_names):
            ...
            for fold in folds:
                ...
                A, alpha = build_adjacency_matrix(embeddings)
                ...
                row = {
                    ...
                    'Inverse_Squared_Distance': alpha,
    ...
    output_csv = f'{model}_{h_or_e}_spectral_analysis_418.csv'
