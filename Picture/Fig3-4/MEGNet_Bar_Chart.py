...
def draw_errorbar(emb_list, cgcnn_list, benchemark, benchemark2, task_name, samples_list, unit_list):
    ...
    plt.savefig("MEGNet_compare_2025_7_21.pdf", bbox_inches='tight', pad_inches=0)
    plt.show()
    plt.clf()
...
if __name__ == '__main__':
    csv_file1 = 'megnet_emb_alltask.csv'
    mae_data1 = modified_csv(csv_file1)
    emb_list = [process_cleaned_data(mae_data1, i) for i in range(8)]

    csv_file3 = 'megnet_cgcnn_alltask.csv'
    mae_data3 = modified_csv(csv_file3)
    cgcnn_list = [process_cleaned_data(mae_data3, i) for i in range(8)]

    benchemark_list = [33.1918, 28.7606, 0.2711, 0.0670, 0.0491, 0.0269, 0.1559, 0.0170]
    megnet_benchmark_list = [54.1719, 28.7606, 0.3391, 0.0871, 0.0668, 0.0352, 0.1934, 0.0252]
    ...
    draw_errorbar(emb_list, cgcnn_list, benchemark_list, megnet_benchmark_list, task_name, samples_list, unit_list)
