...
for fold in "${folds[@]}"; do
    ...
    for combo in "${combinations[@]}"; do
        ...
        # 设置log文件名和路径
        log_name="megnet_${subset}_fold${fold}_dim${dim}_e${a}_f${b}.log"
        log_path="$log_dir/$log_name"

        echo "Running fold=$fold, subset=$subset, dim_node_embed=$dim, a=$a, b=$b on GPU $available_gpu"
        echo "Task $((completed_tasks + 1)) of $total_tasks for fold $fold"

        # 运行任务
        CUDA_VISIBLE_DEVICES="$available_gpu" nohup python megnet_train.py \
            --subset "$subset" \
            --fold "$fold" \
            --dim_node_embed "$dim" \
            --a "$a" \
