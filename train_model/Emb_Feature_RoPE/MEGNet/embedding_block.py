...
class EmbeddingBlock(nn.Module):
    ...
    def __init__(
        ...
        self.binary_linear = nn.Linear(
            in_features=len(pretrain_embeddings[0]),  # bits 长度
            out_features=dim_node_embedding
        )
        self.learn_embed = nn.Embedding(ntypes_node, dim_node_embedding)
    ...
    def forward(self, node_attr, edge_attr, state_attr):
        ...
        if self.ntypes_node is not None:
            ...
            node_feat1 = self.learn_embed(Z_node_attr)

            node_feat2 = self.binary_linear(self.fix_embed(Z_node_attr))

            # 使用 RoPE 旋转 binary embedding
            node_feat2_rot = apply_rope(node_feat2)

            node_feat = self.a * node_feat1 + self.b * node_feat2_rot
