...
class StruData(Dataset):
    ...
    def _precompute_all(self):
        ...
        for idx in tqdm(range(len(self.input)), desc="Precomputing features"):
            crystal = self.input.iloc[idx]
            target = self.output.iloc[idx]

            # 计算原子特征
            elem_ids = np.array([crystal[i].specie.number for i in range(len(crystal))])
            atom_fea = np.vstack([self.ari.get_atom_fea(z) for z in elem_ids])
    ...
    def __getitem__(self, idx):
        ...
        return (atom_fea, nbr_fea, nbr_fea_idx, elem_ids), target
...
def get_train_loader(dataset, collate_fn=default_collate,
    ...
    num_workers = min(16, multiprocessing.cpu_count() // 2)
